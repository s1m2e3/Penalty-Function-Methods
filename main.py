...
for i in range(10):
    ...
    # Plot
    axs[0].plot(vertices[:, 0], vertices[:, 1], 'r-', linewidth=2)
    axs[0].fill(vertices[:, 0], vertices[:, 1], alpha=0.2, color='yellow', label='Rhomboid Region')
    ...
    for i in range(3):
        ...
        dx_1_dt = dH_t@(random_beta_x-eta*A*tanhdphi_1_dbeta_x)+H@(-eta*A*torch.tanh(3*dtanhdphi_1_dbeta_x_dt/A*eta)).diag().unsqueeze(1)
        dx_1_dt_dbeta_x = dH_t@(torch.ones_like(random_beta_x)-eta*A*dtanhdphi_1_dbeta_x_dbeta_x)

        y_1 = H@(random_beta_y-eta*A*tanhdphi_1_dbeta_y)
        dy_1_dt = dH_t@(random_beta_y-eta*A*tanhdphi_1_dbeta_y)+H@(-eta*A*torch.tanh(3*dtanhdphi_1_dbeta_y_dt/A*eta)).diag().unsqueeze(1)
        # dy_1_dt_dbeta_y =
        ...
        axs[0].plot(dx_0_dt.detach().numpy(),dy_0_dt.detach().numpy(),label='dx_0_dt vs dy_0_dt'+str(i),color='g',alpha=0.5+0.1*i)
        axs[0].plot(dx_1_dt.detach().numpy(),dy_1_dt.detach().numpy(),label='dx_1_dt vs dy_1_dt'+str(i),color='b',alpha=0.5+0.1*i)
        axs[0].legend()
